...
<div class="container">
    ...
    <h1 class="text-center" style="color: #fff;">Add Funds</h1>
...
<form action="{{ route('payment.deposit') }}" method="POST" id="payment-form">
    ...
    @if(!$user->stripe_id && !$user->card)
      ...
      @endif
      {{-- @foreach ($user->charges['data'] as $charge)
      {{$charge['amount']/100}}
      @endforeach --}}
    
      <button type="submit" id="depositButton" class="btn btn-green">Make Deposit</button>
    </form>
...
<script>
    ...
    selections.forEach(selection => {
      selection.addEventListener('input', function(e){
        ...
        document.querySelector('#depositButton').innerHTML = 'Make Deposit | $'+val; 
        ...
        <script>
            ...
            selections.forEach(selection => {
              selection.addEventListener('input', function(e){
                ...
                amountNotActive();
                document.querySelector('#da'+val).classList.add('selected');
                document.querySelector('#depositButton').innerHTML = 'Submit Payment | $'+val; 
              });
            });
            ...
            var stripe = Stripe('{{ config('services.stripe.key') }}');
              ...
              const userCard = "{{$user->card_source}}"
